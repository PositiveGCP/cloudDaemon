...
# Segunda parte del cloud servicie que permite conexión con el servidor en Israel
...
# Clase para cada uno de los n objetos que pudieran suceder al leer el archivo de audio
...
class Transaction( object ):
    ...
    def updateBranch( self, information ):
    	try:
            ...
            # TODO: en la versión de produccion quitar la siguiente linea
            # information = "TEST-10|20|30|40"
    	    th = transaction.val()
...
# Clase que obtiene todo el archivo de audio y garantiza un UID para la transaccion
class AudioFile( object ):
    ...
    def __init__( self, uid ):
        self.paths = []
        self.uid = uid
        self.getProcessingFile() # Obtener el archivo de procesamiento automaticamente

    # Obtener el archivo con los audios
    def getProcessingFile( self ):
        try:
            self.audioFile   = storage.child('/cloud/'+self.uid+'/processing.txt').get_url(user['idToken'])
            # print "URL de descarga: -> %s" %(self.audioFile)
            if self.audioFile is not '':
                # Aquí se recibe el link del archivo que contiene ubicación de los audios
                self.process_file( self.audioFile ) # Procesamiento del URL para obtener los audios
    ...
    def process_file( self, URL ):
        try:
            ...
            # TODO: Mejorar el registro de actividades pues pudieron haber sucedido las siguientes:
