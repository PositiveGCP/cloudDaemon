...
user = auth.sign_in_with_email_and_password(
    security.email, security.passwd)  # Iniciar sesion en firebase
...
# Clase firebase que por ahora solo permite revisar las transacciones y comenzar con el kiwi
class Firebase(object):
    ...
    def check_transactions(self):
        ...
        try:
            ...
            print "%20s | %10s | %30s | %30s" % ("UID_transaccion", "Status", "Fecha_inicio", "Fecha_proceso")
            print "-" * 105
            ...
            print "\n"
            msg = "Cantidad de transacciones sin procesar: " + \
                str(count) + " -> " + tr
    ...
    while(True):
        ...
        try:
            ...
            for element in audio_file.paths:
                ...
                transaction_log(transactions_file, voice.response_text, str(
                    voice.status), time.strftime('%Y/%m/%d - %H:%M:%S'))  # Escribir la transaccion
            ...
            msg = "Actualizacion de " + str(item) + ". Procesamiento de " + str(len(
                audio_file.paths)) + " audios. - [" + time.strftime('%Y/%m/%d - %H:%M:%S') + "]. Resultado : " + strres
            system_log(task, msg)
    ...
    if len(uid) > 2:
        ...
        if message["data"] is not None:
            if "processed" in message["data"]:
                # El campo processed si se encuentra por tanto es una transaccion válida
                if message["data"]["processed"] is False:  #  Si no se ha procesado
                    tail.append(uid[1])
                    producer_uids(tail)
                else:
                    pass
...
def main():
    ...
    # ------------------------ Streaming -------------------------
    my_stream = db.child("Transfer").stream(
        producer_stream, stream_id="new_transaction")
    ...
    for i in range(threads_num):
        t1 = threading.Thread(name="Consumidor_de_uids -" +
                              str(i), target=consume_uids, args=())
        t1.start()

    # Crear el kiwi de procesamiento
    if len(unp) is not 0:
        # Es decir que si hay encuestas sin procesar
        t2 = threading.Thread(name="ProducerThread",
                              target=producer_uids, args=(unp,))
        t2.start()
...
if __name__ == "__main__":
    # System: Archivo donde guardar el log del sistema.
